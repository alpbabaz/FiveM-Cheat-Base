...
#include <string>
	...
	void ItemBackground(ImGuiID id, ImRect bb, bool hovered)
	{
		...
		it_anim->second.stroke = ImLerp(it_anim->second.stroke, hovered ? ImColor(1.f, 1.f, 1.f, 0.1f) : ImColor(1.f, 1.f, 1.f, 0.05f), c::anim::speed);

		ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(it_anim->second.stroke), c::elements::rounding);
	}
	...
	void ItemDescription(const char* text, ImGuiID id, ImRect bb, bool hovered, ImVec2 pos, ImVec4 description_col)
	{
		...
		it_anim->second.hint_text_col = ImLerp(it_anim->second.hint_text_col, it_anim->second.is_hinted ? utils::GetColorWithAlpha(label::defaultColor, ImGui::GetStyle().Alpha) : utils::GetColorWithAlpha(label::defaultColor, 0.f), GetAnimSpeed() / 3);
		...
		it_anim->second.hint_text_col = ImLerp(it_anim->second.hint_text_col, it_anim->second.is_hinted ? utils::GetColorWithAlpha(label::defaultColor, ImGui::GetStyle().Alpha) : utils::GetColorWithAlpha(label::defaultColor, 0.f), GetAnimSpeed());
	...
	bool Keybind(const char* label, const char* description, int* key, int* mode)
	{
		...
		const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(width, 55));
 ...
 hovered ? c::anim::defaultColor : c::anim::defaultColor, io.DeltaTime * 5.f);
		it_anim->second.text = ImLerp(it_anim->second.text, hovered ? c::text::label::hovered : c::text::label::defaultColor, io.DeltaTime * 5.f);

		it_anim->second.text_color = ImLerp(it_anim->second.text_color, it_anim->second.status != KEYBIND_NONE ? c::text::label::active : hovered ? c::text::label::hovered : c::text::label::defaultColor, GetAnimSpeed());
		it_anim->second.description_col = ImLerp(it_anim->second.description_col, it_anim->second.status != KEYBIND_NONE ? c::text::description::active : hovered ? c::text::description::hovered : c::text::description::defaultColor, GetAnimSpeed());


		ItemBackground(id, total_bb, hovered);
		...
		GetWindowDrawList()->AddText(ImVec2(total_bb.Min.x + 14.f, total_bb.GetCenter().y - CalcTextSize(label).y), GetColorU32(it_anim->second.text_color), label);

		ImRect descrtiption_bb(ImVec2(total_bb.Min.x + 14.f, total_bb.GetCenter().y), ImVec2(total_bb.Min.x + 14.f, total_bb.GetCenter().y) + CalcTextSize(description));
		ItemDescription(description, id, ImRect{ ImVec2(key_bb.Min.x - 25, descrtiption_bb.Min.y), ImVec2(key_bb.Min.x - 5, descrtiption_bb.Max.y) }, hovered, ImVec2(total_bb.Min.x + 14.f, total_bb.GetCenter().y), it_anim->second.description_col);
		...
		GetWindowDrawList()->AddText(key_bb.GetCenter() - key_text_size * 0.5f, ImColor(1.f, 1.f, 1.f, it_anim->second.alpha), it_anim->second.key_name);
	...
	bool MiniBind(const char* label, int* key, int* mode)
	{
		...
		it_anim->second.background = ImLerp(it_anim->second.background, g.ActiveId == id ? c::anim::active : c::anim::defaultColor, GetAnimSpeed());
	...
	bool ChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool cap, ImGuiWindowFlags flags)
	{
		...
		ImVec2 size = ImFloor(size_arg) + ImVec2(0, cap ? 30 : 0);
		...
		SetNextWindowPos(ImVec2(parent_window->DC.CursorPos + ImVec2(0, cap ? 30 : 0)));
		SetNextWindowSize(size - ImVec2(0, cap ? 30 : 0));
		...
		GetWindowDrawList()->AddText(parent_window->DC.CursorPos + ImVec2(GetStyle().WindowPadding), utils::GetColorWithAlpha(c::label::defaultColor, ImGui::GetStyle().Alpha), name);
	...
	bool Button(const char* label, const ImVec2& size_arg)
	{
		...
		it_anim->second.background = ImLerp(
			...
			IsItemActive() || hovered ? ImColor(c::anim::active) : ImColor(c::elements::background),
			g.IO.DeltaTime * 6.f
		);
		it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() || hovered ? c::label::active : c::label::defaultColor, g.IO.DeltaTime * 6.f);

		GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.background), c::page::rounding);
		GetWindowDrawList()->AddRect(bb.Min, bb.Max, c::stroke_color, c::page::rounding);
	...
	bool Tab(const char* label, const char* icon, int* v, int number) {
		...
		const ImRect total_bb(pos, pos + ImVec2(c::sidebar::width - 20, c::sidebar::item_height));
		...
		bool isActive = *v == number;

		it_anim->second.frame_col = ImLerp(it_anim->second.frame_col,
			isActive ? ImVec4(utils::GetColorWithAlpha(c::sidebar::active_border, 0.15f)) :
			hovered ? ImVec4(utils::GetColorWithAlpha(c::sidebar::item_hover_bg, 0.5f)) :
			ImVec4(0, 0, 0, 0), GetAnimSpeed());

		it_anim->second.text_col[0] = ImLerp(it_anim->second.text_col[0], ImVec4(isActive ? c::sidebar::text_active : (hovered ? c::sidebar::text_hover : c::sidebar::text_default)), GetAnimSpeed());

		it_anim->second.text_col[1] = ImLerp(it_anim->second.text_col[1], ImVec4(isActive ? c::sidebar::text_active : (hovered ? c::sidebar::text_hover : c::sidebar::text_default)), GetAnimSpeed());

		it_anim->second.icon_col = ImLerp(it_anim->second.icon_col, ImVec4(isActive ? c::sidebar::text_active : (hovered ? c::sidebar::text_hover : c::sidebar::text_default)), GetAnimSpeed());

		window->DrawList->AddRectFilled(total_bb.Min, total_bb.Max, GetColorU32(ImVec4(it_anim->second.frame_col)), 10.f);

		if (isActive) {
			window->DrawList->AddRect(total_bb.Min, total_bb.Max, GetColorU32(ImVec4(utils::GetColorWithAlpha(c::sidebar::active_border, 0.4f))), 10.f, 0, 1.5f);
		}

		ImVec2 icon_center = ImVec2(total_bb.Min.x + 24, total_bb.GetCenter().y);
		PushFont(font::icomoon_page);
		window->DrawList->AddText(ImVec2(icon_center.x - CalcTextSize(icon).x / 2, icon_center.y - CalcTextSize(icon).y / 2),
			GetColorU32(it_anim->second.icon_col), icon);
		PopFont();

		PushFont(font::tab_font);
		ImVec2 text_pos = ImVec2(total_bb.Min.x + 44, utils::center_text(total_bb.Min, total_bb.Max, label).y);
		window->DrawList->AddText(text_pos, GetColorU32(it_anim->second.text_col[0]), label);
		PopFont();
	...
	static constexpr float CB_SMOKE_LOOP = 8.0f;

	void DrawCheckboxSmoke(ImDrawList* dl, ImRect bb) {
		ID3D11ShaderResourceView* tex = smoke::GetTexture();
		if (!tex) return;
	...
	bool Checkbox(const char* label, const char* description, bool* v)
	{
		...
		const ImRect total_bb(pos, pos + ImVec2(w, 56));
		...
		it_anim->second.rect_color = ImLerp(it_anim->second.rect_color, *v ? c::toggle::bg_on : c::toggle::bg_off, GetAnimSpeed());
		it_anim->second.text_color = ImLerp(it_anim->second.text_color, *v ? c::text::label::active : hovered ? c::text::label::hovered : c::text::label::defaultColor, GetAnimSpeed());
		it_anim->second.description_col = ImLerp(it_anim->second.description_col, *v ? c::text::description::active : hovered ? c::text::description::hovered : c::text::description::defaultColor, GetAnimSpeed());

		ItemBackground(id, total_bb, hovered);

		if (*v) {
			DrawCheckboxSmoke(GetWindowDrawList(), total_bb);
		}

		GetWindowDrawList()->AddText(ImVec2(total_bb.Min.x + 14.f, total_bb.GetCenter().y - CalcTextSize(label).y / 2), GetColorU32(it_anim->second.text_color), label);

		ImRect descrtiption_bb(ImVec2(total_bb.Min.x + 14.f, total_bb.GetCenter().y + CalcTextSize(label).y / 2 + 2), ImVec2(total_bb.Min.x + 14.f, total_bb.GetCenter().y + CalcTextSize(label).y / 2 + 2) + CalcTextSize(description));
		ItemDescription(description, id, ImRect{ ImVec2(total_bb.Max.x - 80, descrtiption_bb.Min.y), ImVec2(total_bb.Max.x - 5, descrtiption_bb.Max.y) }, hovered, ImVec2(total_bb.Min.x + 14.f, total_bb.GetCenter().y + CalcTextSize(label).y / 2 + 2), it_anim->second.description_col);

		float toggle_w = c::toggle::width;
		float toggle_h = c::toggle::height;
		float toggle_x = total_bb.Max.x - toggle_w - 14.f;
		float toggle_y = total_bb.GetCenter().y - toggle_h / 2;
		ImRect toggle_bb(ImVec2(toggle_x, toggle_y), ImVec2(toggle_x + toggle_w, toggle_y + toggle_h));

		GetWindowDrawList()->AddRectFilled(toggle_bb.Min, toggle_bb.Max, GetColorU32(it_anim->second.rect_color), toggle_h / 2);

		if (*v) {
			GetWindowDrawList()->AddShadowCircle(ImVec2(toggle_x + toggle_w - c::toggle::circle_radius, toggle_y + toggle_h / 2), c::toggle::circle_radius + 2.f, GetColorU32(it_anim->second.rect_color), 12.f, ImVec2(0, 0));
		}

		float circle_x = ImLerp(toggle_x + c::toggle::circle_radius + 2.f, toggle_x + toggle_w - c::toggle::circle_radius - 2.f, it_anim->second.checkbox_alpha);
		ImVec4 circle_col = *v ? ImVec4(c::toggle::circle_on) : ImVec4(c::toggle::circle_off);
		GetWindowDrawList()->AddCircleFilled(ImVec2(circle_x, toggle_y + toggle_h / 2), c::toggle::circle_radius, GetColorU32(circle_col), 60);
	...
	bool BeginCombo(const char* label, const char* preview_value, int val, bool multi, ImGuiComboFlags flags)
	{
		...
		const ImRect total_bb(pos, pos + ImVec2(w, 45));
		...
		it_anim->second.text = ImLerp(it_anim->second.text, it_anim->second.opened_combo ? c::text::label::active : hovered ? c::text::label::hovered : c::text::label::defaultColor, GetAnimSpeed());
		it_anim->second.background = ImLerp(
			it_anim->second.background,
			it_anim->second.opened_combo ? ImColor(c::second_color) : ImColor(c::elements::background),
			g.IO.DeltaTime * 6.f
		);
		...
		GetWindowDrawList()->AddText(ImVec2(total_bb.Min.x + 14.f, utils::center_text(total_bb.Min, total_bb.Max, label).y), GetColorU32(it_anim->second.text), label);

		PushClipRect(bb.Min, bb.Max, true);
		GetWindowDrawList()->AddText(ImVec2(bb.Max.x - (34.f + CalcTextSize(preview_value).x), utils::center_text(total_bb.Min, total_bb.Max, preview_value).y), utils::GetColorWithAlpha(c::label::active, style.Alpha), preview_value);
		PopClipRect();

		ImRotateStart();
		GetWindowDrawList()->AddText(ImVec2(bb.Max.x - 21 - CalcTextSize(ICON_DOWN_SMALL_LINE).x / 2, bb.GetCenter().y - CalcTextSize(ICON_DOWN_SMALL_LINE).y / 2), c::text::label::defaultColor, ICON_DOWN_SMALL_LINE);
		...
		if (window_pos.x + window_size.x > viewport_pos.x + viewport_size.x) {
			window_pos.x = (viewport_pos.x + viewport_size.x) - window_size.x; 
		}

		
		if (window_pos.y + window_size.y > viewport_pos.y + viewport_size.y) {
			window_pos.y = (viewport_pos.y + viewport_size.y) - window_size.y; 
		}
	...
	bool Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
	{
		...
		it_anim->second.text = ImLerp(it_anim->second.text, selected ? c::label::active : c::label::defaultColor, GetAnimSpeed());
		...
		it_anim->second.stroke = ImLerp(
			it_anim->second.stroke,
			selected ? ImColor(c::child::stroke) : ImColor(utils::GetColorWithAlpha(c::child::stroke, 0.f)),
			GetAnimSpeed()
		);
	...
	bool ColorEdit4(const char* label, const char* description, float col[4], ImGuiColorEditFlags flags)
	{
		...
		const ImRect rect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(width, 55));
		...
		if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
		{
			...
			if (BeginPopup("picker", ImGuiWindowFlags_NoBackground))
			{
				...
				ImGui::GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(c::elements::background), c::elements::rounding);
		...
		if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
		{
			...
			it_anim->second.text = ImLerp(it_anim->second.text, ImGui::IsPopupOpen("picker") ? c::text::label::active : IsMouseHoveringRect(rect.Min, rect.Max, true) ? c::text::label::hovered : c::text::label::defaultColor, GetAnimSpeed());

			it_anim->second.icon = ImLerp(it_anim->second.icon, ImGui::IsPopupOpen("picker") ? c::anim::active : IsMouseHoveringRect(rect.Min, rect.Max, true) ? c::text::label::hovered : c::text::label::defaultColor, GetAnimSpeed());

			it_anim->second.text_color = ImLerp(it_anim->second.text_color, ImGui::IsPopupOpen("picker") ? c::text::label::active : rect.Contains(ImGui::GetMousePos()) ? c::text::label::hovered : c::text::label::defaultColor, GetAnimSpeed());
			it_anim->second.description_col = ImLerp(it_anim->second.description_col, ImGui::IsPopupOpen("picker") ? c::text::description::active : rect.Contains(ImGui::GetMousePos()) ? c::text::description::hovered : c::text::description::defaultColor, GetAnimSpeed());
			...
			GetWindowDrawList()->AddText(ImVec2(rect.Min.x + 14.f, rect.GetCenter().y - CalcTextSize(label).y), GetColorU32(it_anim->second.text_color), label);
			ImGui::PopClipRect();
			...
			ItemDescription(description, GetID(label), ImRect{ ImVec2(check_bb.Min.x - 25, descrtiption_bb.Min.y), ImVec2(check_bb.Min.x - 5, descrtiption_bb.Max.y) }, rect.Contains(ImGui::GetMousePos()), ImVec2(rect.Min.x + 14.f, rect.GetCenter().y), it_anim->second.description_col);
	...
	bool KnobScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
	{
		...
		it_anim->second.text = ImLerp(it_anim->second.text, g.ActiveId == id ? c::text::label::active : hovered ? c::text::label::hovered : c::text::label::defaultColor, g.IO.DeltaTime * 6.f);
		...
		GetWindowDrawList()->AddText(ImVec2(frame_bb.Max.x - (40 + CalcTextSize(value_buf).x), frame_bb.Min.y + (32 - CalcTextSize(value_buf).y) / 2), c::text::label::defaultColor, value_buf);
	...
	bool SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
	{
		...
		const ImRect total_bb(pos, pos + ImVec2(w, 50));
		const ImRect item_bb(total_bb.Min, total_bb.Max);

		const ImRect slider_bb(total_bb.Min + ImVec2(15, 15 + CalcTextSize(label).y), total_bb.Max - ImVec2(15, 13));
		...
		it_anim->second.text_color = ImLerp(it_anim->second.text_color, held ? c::text::label::active : hovered ? c::text::label::hovered : c::text::label::defaultColor, GetAnimSpeed());

		GetWindowDrawList()->AddRectFilled(slider_bb.Min, slider_bb.Max, utils::GetColorWithAlpha(c::anim::defaultColor, style.Alpha), c::elements::rounding);
		GetWindowDrawList()->AddShadowRect(slider_bb.Min, ImVec2(it_anim->second.slow + (slider_bb.Min.x + 3), slider_bb.Max.y), utils::GetColorWithAlpha(c::anim::active, style.Alpha / 2), 35.f, ImVec2(0, 0), 0, c::elements::rounding);
		GetWindowDrawList()->AddRectFilled(slider_bb.Min, ImVec2(it_anim->second.slow + (slider_bb.Min.x + 6), slider_bb.Max.y), utils::GetColorWithAlpha(c::anim::active, style.Alpha), c::elements::rounding);

		GetWindowDrawList()->AddCircleFilled(ImVec2(it_anim->second.slow + (slider_bb.Min.x + 6) - slider_bb.GetSize().y / 2, slider_bb.GetCenter().y), it_anim->second.circle_radius, utils::GetColorWithAlpha(c::label::active, style.Alpha), 36.f);
		GetWindowDrawList()->AddCircleFilled(ImVec2(it_anim->second.slow + (slider_bb.Min.x + 6) - slider_bb.GetSize().y / 2, slider_bb.GetCenter().y), it_anim->second.circle_radius / 1.5f, utils::GetColorWithAlpha(c::label::defaultColor, style.Alpha), 36.f);

		GetWindowDrawList()->AddText(ImVec2(total_bb.Min.x + 14.f, total_bb.GetCenter().y - CalcTextSize(label).y), GetColorU32(it_anim->second.text_color), label);

		GetWindowDrawList()->AddText(ImVec2(slider_bb.Max.x - CalcTextSize(value_buf).x, total_bb.GetCenter().y - CalcTextSize(label).y), GetColorU32(it_anim->second.text_color), value_buf);
