...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
	...
	it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? c::anim::active : hovered ? c::text::label::hovered : c::text::label::defaultColor, g.IO.DeltaTime * 6.f);
	...
	if (render_cursor || render_selection)
	{
		...
		{
	ImU32 col = is_displaying_hint ? c::label::defaultColor : c::label::active;
	...
	it_anim->second.label = ImLerp(it_anim->second.label, CalcTextSize(buf_display).x < 1.f ? c::text::label::defaultColor: utils::GetColorWithAlpha(c::text::label::defaultColor, 0.f), g.IO.DeltaTime * 6.f);
