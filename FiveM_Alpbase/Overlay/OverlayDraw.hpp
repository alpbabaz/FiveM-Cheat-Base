...
void DrawLineOutline(ImVec2 a, ImVec2 b, ImColor color, float width) {
	...
	ImVec4 outlineColorDark = ImVec4(0, 0, 0, 0.8f);
...
void DrawShadowedTexts(ImDrawList* drawList, const std::string& text, const ImVec2& pos, float size, ImU32 color, bool center) {
	...
	ImU32 strokeColor = ImGui::GetColorU32(ImVec4(0, 0, 0, 1.0f));

	float y = 0.0f;
	int index = 0;
	while (std::getline(stream, line)) {
		ImVec2 textSize = pFont->CalcTextSizeA(size * 0.8f, FLT_MAX, 0.0f, line.c_str());
		ImVec2 textPos;
		...
		drawList->AddText(pFont, size * 0.8f, textPos + ImVec2(-1, -1), strokeColor, line.c_str());
		drawList->AddText(pFont, size * 0.8f, textPos + ImVec2(1, -1), strokeColor, line.c_str());
		drawList->AddText(pFont, size * 0.8f, textPos + ImVec2(-1, 1), strokeColor, line.c_str());
		drawList->AddText(pFont, size * 0.8f, textPos + ImVec2(1, 1), strokeColor, line.c_str());
		drawList->AddText(pFont, size * 0.8f, textPos, color, line.c_str());
		y = pos.y + textSize.y * (index + 1);
		index++;
	}
}
...
void DrawGradientRect(ImDrawList* drawList, ImVec2 topLeft, ImVec2 bottomRight, ImColor startColor, ImColor endColor) {
	...
	for (int i = 0; i < steps; i++) {
		...
		ImColor currentColor = ImColor(
			(int)(startColor.Value.x * 255.0f * (0.50f - t) + endColor.Value.x * 255.0f * t),
			(int)(startColor.Value.y * 255.0f * (0.25f - t) + endColor.Value.y * 255.0f * t),
			(int)(startColor.Value.z * 255.0f * (0.50f - t) + endColor.Value.z * 255.0f * t),
			(int)(startColor.Value.w * 255.0f * (0.25f - t) + endColor.Value.w * 255.0f * t)
		);
		ImVec2 lineTop(topLeft.x, topLeft.y + t * height);
		ImVec2 lineBottom(bottomRight.x, topLeft.y + t * height + height / steps);
		drawList->AddRectFilled(lineTop, lineBottom, currentColor);
	}
}